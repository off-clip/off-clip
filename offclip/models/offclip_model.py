...
class OFFCLIP(nn.Module):
    def __init__(self, cfg):
        super(OFFCLIP, self).__init__()
        ...
        self.local_loss = loss.offclip_loss.local_loss
        self.global_loss = loss.offclip_loss.global_loss
        self.local_loss_weight = cfg.model.offclip.local_loss_weight
        self.global_loss_weight = cfg.model.offclip.global_loss_weight
        self.ce_loss_weight = cfg.model.offclip.ce_loss_weight

        self.temp1 = cfg.model.offclip.temp1
        self.temp2 = cfg.model.offclip.temp2
        self.temp3 = cfg.model.offclip.temp3
    ...
    def forward_w_attn(self, x):
        ...
        i2t_cls, i2t_attn_map = self.fusion_module(torch.cat([img_emb_g.unsqueeze(1) , img_emb_l_], dim=1) , text_emb_g, return_atten=True)
        # print('i2t')
        # print(len(i2t_cls), len(i2t_attn_map))
        # print(i2t_cls[0].shape)
        # print(i2t_attn_map[0].shape)
        ...
        text_emb_l_ = text_emb_l_.permute(0, 2, 1) #patch_num b dim # [97, 512, 768]
        # 
        t2i_cls, t2i_attn_map = self.fusion_module(torch.cat([text_emb_g.unsqueeze(1) , text_emb_l_], dim=1) , img_emb_g, return_atten=True)
        # print('t2i')
        # print(len(t2i_cls), len(t2i_attn_map))
        # print(t2i_cls[0].shape)
        # print(t2i_attn_map[0].shape)
        t2i_cls = t2i_cls.squeeze(-1)
 
        # 
        return img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, i2t_cls, t2i_cls, i2t_attn_map, t2i_attn_map
    ...
    def process_img(self, paths, dicom_true, device):
        ...
        for p in paths:
            # print(f'IN MODEL!! p: {p}')
            if dicom_true:
                ds = pydicom.dcmread(p)
                dcm_arr = ds.pixel_array.astype(float)
                scaled_image = (np.maximum(dcm_arr, 0) / dcm_arr.max()) * 255.0
                x = np.uint8(scaled_image)
                if x is None:
                    print(f"ERROR: Could not read image: {p}")  # Catch problematic images
                    continue
